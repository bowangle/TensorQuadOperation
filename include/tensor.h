...
// =================================================
//
// FILE FORMAT (ARMA_CUB_TXT_FC016, Armadillo native cube text format):
//   An Armadillo cube (n_rows, n_cols, n_slices) is written slice by slice.
//   Within each slice there is one text line per ROW, with the COLUMNS
//   printed across the line. Token order on disk is therefore:
//       slice (slowest)  ->  row  ->  col (fastest)
//   With our mapping  rows = n_left, cols = n_phys, slices = n_right,
//   the on-disk element order is:
//       id_right (slowest) -> id_left -> id_phys (fastest)
//
//   BUGFIX: the previous loader/writers iterated (left, phys, right) with
//   right fastest. Dimensions and token counts matched, so loading
//   "succeeded", but every core had its elements permuted: the loaded TT
//   was a valid tensor train, self-consistent under all algebraic tests,
//   yet unrelated to the function saved by TCI (full rank chi=16 instead
//   of ~8, evaluations reaching ~1e5 instead of |f| <= 1.5). All three
//   routines below now use the Armadillo order so that files written by
//   Armadillo's cube.save(), by save_Tensor3D_to_arma, and read by
//   load_vector_tensor are mutually compatible.
//
...
std::vector<Tensor3D<T>> load_vector_tensor(std::istream& in)
{
    ...
    for (Eigen::Index t = 0; t < nBit; ++t)
    {
        ...
        in >> r >> c >> s;
        Tensor3D<T> X(r, c, s);
        // Armadillo cube order: slice -> row -> col, i.e. right -> left -> phys
    ...
    const std::vector<Tensor3D<T>>& Xs)
{
    ...
    for (std::size_t t = 0; t < Xs.size(); ++t)
    {
        ...
        // Armadillo cube order: slice -> row -> col, i.e. right -> left -> phys.
        // One line per (slice, row) pair, columns across the line.
        for (Eigen::Index id_right = 0; id_right < X.n_right; ++id_right)
        {
            for (Eigen::Index id_left = 0; id_left < X.n_left; ++id_left)
            {
